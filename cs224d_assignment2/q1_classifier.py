...
class SoftmaxModel(Model):
  ...
  def add_placeholders(self):
    ...
    self.input_placeholder = tf.placeholder(
        tf.float32, shape=(config.batch_size, config.n_features), name='X')
    self.labels_placeholder = tf.placeholder(
        tf.float32, shape=(config.batch_size, config.n_classes), name='y')
    # END YOUR CODE
  ...
  def add_training_op(self, loss):
    ...
    """
    # YOUR CODE HERE
    train_op = tf.train.GradientDescentOptimizer(learning_rate = self.config.lr or 1e-3).minimize(loss)
    # END YOUR CODE
    return train_op
  ...
  def add_model(self, input_data):
    ...
    config = self.config
    W = tf.Variable(
        tf.zeros([config.n_features, config.n_classes], tf.float32), name='W')
    b = tf.Variable(tf.zeros(config.n_classes, tf.float32), name='b')
