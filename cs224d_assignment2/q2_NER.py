...
class NERModel(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    self.input_placeholder = tf.placeholder(tf.int32, shape=(
        None, config.window_size), name='X')
    self.labels_placeholder = tf.placeholder(
        tf.float32, shape=(None, config.label_size), name='y')
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope('Layer'):
      W = tf.get_variable('W',
                          shape=[config.window_size * config.embed_size, config.hidden_size], initializer=xavier_initializer)
      b1 = tf.Variable(xavier_initializer((config.hidden_size,)))

    with tf.variable_scope('Softmax'):
      U = tf.get_variable('U', shape=[
                          config.hidden_size, config.label_size], initializer=xavier_initializer)
      b2 = tf.Variable(xavier_initializer((config.label_size,)))

    X = tf.nn.relu(tf.matmul(window, W) + b1)
    if config.dropout is not None:
      X = tf.nn.dropout(X, keep_prob=self.dropout_placeholder)
    output = tf.matmul(X, U) + b2

    tf.add_to_collection('ssw', tf.nn.l2_loss(W))
    # tf.add_to_collection('ssw', tf.nn.l2_loss(b1))
    tf.add_to_collection('ssw', tf.nn.l2_loss(U))
    # tf.add_to_collection('ssw', tf.nn.l2_loss(b2))
    # END YOUR CODE
    return output
  ...
  def add_loss_op(self, y):
    ...
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
        labels=self.labels_placeholder, logits=y)) + self.config.l2 * tf.add_n(tf.get_collection('ssw'))
  ...
  def run_epoch(self, session, input_data, input_labels,
    ...
    # We're interested in keeping track of the loss and accuracy during
    # training
    total_loss = []
    ...
    with tf.Session() as session:
      ...
      for epoch in xrange(config.max_epochs):
        ...
        val_loss, predictions = model.predict(
            session, model.X_dev, model.y_dev)
